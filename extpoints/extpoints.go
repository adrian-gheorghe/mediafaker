...
var MediaFakerTypes = &mediaFakerTypeExt{
	newExtensionPoint(new(MediaFakerType)),
}

type mediaFakerTypeExt struct {
	*extensionPoint
}

func (ep *mediaFakerTypeExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *mediaFakerTypeExt) Register(extension MediaFakerType, name string) bool {
	return ep.register(extension, name)
}

func (ep *mediaFakerTypeExt) Lookup(name string) MediaFakerType {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
	return ext.(MediaFakerType)
}

func (ep *mediaFakerTypeExt) Select(names []string) []MediaFakerType {
	var selected []MediaFakerType
...
func (ep *mediaFakerTypeExt) All() map[string]MediaFakerType {
	all := make(map[string]MediaFakerType)
	for k, v := range ep.all() {
		all[k] = v.(MediaFakerType)
	}
	return all
}

func (ep *mediaFakerTypeExt) Names() []string {
	var names []string
	for k := range ep.all() {
